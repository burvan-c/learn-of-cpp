...
//	}
	...
	//.h文件写函数声明，.cpp文件写函数定义
	//32位操作系统，指针占4个字节；64位操作系统，指针占8个字节
	//空指针：指针变量指向编号为0的空间int*p=NULL,空指针不可访问
	//野指针：指针变量指向非法内存空间，不可访问
	//指针常量：int*const p=&a 指针的指向不可改，指向的值可以改
	//常量指针：const int* p=&a  指针的指向可改，指向的值都不可改
	//          const int* const p=&a  都不可改
